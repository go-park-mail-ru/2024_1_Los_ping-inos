...
func processInterestIDFilter(filter *profile.InterestGetFilter, whereMap *qb.And) {
	if filter.ID != nil {
		*whereMap = append(*whereMap, qb.Eq{"id": filter.ID})
	}
}

func processInterestNameFilter(filter *profile.InterestGetFilter, whereMap *qb.And) {
	if filter.Name != nil {
		*whereMap = append(*whereMap, qb.Eq{"name": filter.Name})
	}
}

func (storage *InterestStorage) Get(ctx context.Context, filter *profile.InterestGetFilter) ([]*profile.Interest, error) {
	...
	if filter == nil {
		filter = &profile.InterestGetFilter{}
	}
	...
	interests := make([]*profile.Interest, 0)
	for rows.Next() {
...
func (storage *InterestStorage) GetPersonInterests(ctx context.Context, personID types.UserID) ([]*profile.Interest, error) {
	logger := ctx.Value(Logg).(*Log)
	...
	return storage.Get(ctx, &profile.InterestGetFilter{ID: ids})
}
