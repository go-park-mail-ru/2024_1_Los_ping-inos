...
func (deliver *FeedHandler) CreateLike() func(respWriter http.ResponseWriter, request *http.Request) {
	return func(respWriter http.ResponseWriter, request *http.Request) {
		...
		likes, err := deliver.usecase.CreateLike(request.Context().Value(RequestUserID).(types.UserID), requestBody.Profile2, request.Context())
		if err != nil {
		...
		requests.SendResponse(respWriter, request, http.StatusOK, likes)
